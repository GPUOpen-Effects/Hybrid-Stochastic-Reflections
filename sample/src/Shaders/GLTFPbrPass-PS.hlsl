...
Output mainPS(VS_OUTPUT_SCENE Input, bool bIsFontFacing : SV_IsFrontFace)
{
    ...
    emission = (emissiveTexture.Sample(samEmissive, getEmissiveUV(Input))).rgb * u_pbrParams.myPerObject_u_EmissiveFactor.rgb * myPerFrame.u_EmissiveFactor;
#else        
    emission = u_pbrParams.myPerObject_u_EmissiveFactor.rgb * myPerFrame.u_EmissiveFactor;
#endif

#ifdef ID_baseColorTexture
    output.diffuseColor = baseColorTexture.SampleBias(samBaseColor, getBaseColorUV(Input), myPerFrame.u_LodBias);
#else // ID_baseColorTexture
    output.diffuseColor = (0.0).xxxx;
#endif // ID_baseColorTexture

    float emission_k = saturate(max(emission.x, max(emission.y, emission.z)));
    output.diffuseColor.xyz = lerp(output.diffuseColor.xyz, emission / 4.0, emission_k);
    output.diffuseColor.w   = emission_k;
